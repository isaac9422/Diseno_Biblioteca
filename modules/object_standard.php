...
class object_standard {
    ...
    function __construct($data = NULL, $components = NULL, $orm = NULL, $auxiliars = NULL) {
        if ($data != NULL) {
            $this->set_attributes($data, $auxiliars[get_class($this)]);
        }
        if ($components[get_class($this)] != NULL) {
            $this->assign_components($components, $orm, $auxiliars);
        }
    }
    ...
    function set_attributes($data, $auxiliars = NULL) {
        ...
        if ($auxiliars != NULL) {
            foreach ($auxiliars as $attribute_aux) {
                if (!is_empty($data->$attribute_aux)) {
                    $this->auxiliars[$attribute_aux] = $data->$attribute_aux;
                }
            }
        }
    }
                ...
                while ($data != NULL) {
                    ...
                    if (get_class($this) == $class_ref) { //when is a foreign to the own table
                        ...
                        if (!is_empty($rel)) {
                            ...
                            while ($ft < count($rel)) {
                                $rel_one = $rel_one . $data->$rel[$ft];
                                $rel_aux[] = $attribute_info[$rel[$ft]]['foreign_attribute'];
                                $ft++;
                            }$ft = 0;
                            while ($ft < count($rel_aux)) {
                                $rel_two = $rel_two . $this->get($rel_aux[$ft]);
                                $ft++;
                            }
                        ...
                        if (is_empty($rel) && is_empty($rel_this)) {
                            ...
                            if (is_empty($rel)) {
                                $attribute_info = $this->metadata();
                                while ($f < count($rel_this)) {
                                    $rel_one = $rel_one . $this->get($rel_this[$f]);
                                    $rel_aux[] = $attribute_info[$rel_this[$f]]['foreign_attribute'];
                                    $f++;
                                }$f = 0;
                                while ($f < count($rel_aux)) {
                                    $rel_two = $rel_two . $data->$rel_aux[$f];
                                    $f++;
                                }
                            } else {
